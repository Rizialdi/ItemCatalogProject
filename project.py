from flask import Flask, render_template, url_for, request, redirect, session as sess, jsonify
...
def showItemDescription(categorie, item):
    ...
    #condition to check whether someone first visit the website or 
    #if he did but logged out after 
    if 'logged_in' not in sess or not sess['logged_in']:
...
def new_user():
    if request.method == 'POST':
        ...
        if session.query(User).filter_by(username = username).first() is not None:
            ...
            return render_template('register.html', error_message =  error_message) #in these error cases return blank form
        ...
        return redirect(url_for('signin_user', usrname = username))
    else:
        return render_template('register.html')  #in this case return only a blank form
...
def signin_user():
    if request.method == 'POST':
        ...
        if username is None or password is None:
            error_message =  "missing arguments"
            return render_template('signin.html', error_message = error_message)  #in this case return only a blank form
        ...
        return render_template('signin.html', usrname = request.args.get('usrname'))  #in this case return only a blank form
...
def cat_json():
    ...
    #find a way to mix these 2 queries
